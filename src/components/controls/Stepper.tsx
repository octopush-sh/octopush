...
export function Stepper({ value, min = 1, max = 9, step = 1, onChange, ariaLabel }: Props) {
  return (
    <div aria-label={ariaLabel} className="inline-flex shrink-0 items-center rounded-md border border-octo-hairline bg-octo-onyx">
      ...
      <span className="octo-tabular min-w-[2.5rem] px-1 text-center font-mono text-[11px] text-octo-ivory">{value}</span>
      <button
        type="button"
        aria-label="Increase"
        disabled={value >= max}
