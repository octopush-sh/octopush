...
describe("PipelineSetup begin gate", () => {
  it("disables Begin + shows the helper when a run is executing", () => {
    render(<PipelineSetup workspaceId="w1" defaultTask="build it" onBegin={vi.fn()} executingRun onEditPipeline={vi.fn()} />);
  ...
  it("enables Begin when no run is executing and a task is set", () => {
    render(<PipelineSetup workspaceId="w1" defaultTask="build it" onBegin={vi.fn()} executingRun={false} onEditPipeline={vi.fn()} />);
...
describe("PipelineSetup budget field", () => {
  it("renders the optional budget input with its quiet eyebrow", () => {
    render(<PipelineSetup workspaceId="w1" defaultTask="t" onBegin={vi.fn()} executingRun={false} onEditPipeline={vi.fn()} />);
  ...
  it("passes the parsed budget to onBegin", () => {
    const onBegin = vi.fn();
    render(<PipelineSetup workspaceId="w1" defaultTask="build it" onBegin={onBegin} executingRun={false} onEditPipeline={vi.fn()} />);
...
describe("PipelineSetup designed states", () => {
  it("renders the ceremony header", () => {
    render(<PipelineSetup workspaceId="w1" defaultTask="" onBegin={vi.fn()} executingRun={false} onEditPipeline={vi.fn()} />);
  ...
  it("shows skeletons while pipelines load, not the error card", () => {
    ...
    const { container } = render(
      <PipelineSetup workspaceId="w1" defaultTask="" onBegin={vi.fn()} executingRun={false} onEditPipeline={vi.fn()} />,
    );
  ...
  it("the beacon lands on Begin only when ready (Law 2)", () => {
    const { container, rerender } = render(
      <PipelineSetup workspaceId="w1" defaultTask="build it" onBegin={vi.fn()} executingRun={false} onEditPipeline={vi.fn()} />,
    );
    expect(container.querySelectorAll(".octo-stage-pulse")).toHaveLength(1); // the CTA
    rerender(<PipelineSetup workspaceId="w1" defaultTask="build it" onBegin={vi.fn()} executingRun onEditPipeline={vi.fn()} />);
...
describe("PipelineSetup launcher prefill (R3)", () => {
  ...
  it("applies task + pipeline + overrides from the prefill, then clears it", () => {
    ...
    const onBegin = vi.fn();
    render(<PipelineSetup workspaceId="w1" defaultTask="" onBegin={onBegin} executingRun={false} onEditPipeline={vi.fn()} />);
