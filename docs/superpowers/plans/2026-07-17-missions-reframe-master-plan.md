...
- **M4.2 — design ~~(release 15)~~ RETIRADA / SUBSUMIDA por prompt-genesis G5 (el Sketchbook, enviado v0.4.35).** El diseño original (misiones sin workspace: `workspace_id=NULL`, Rail discontinuo, superficie de artefactos nueva) chocaba con el modelo workspace-keyed. El Sketchbook lo reemplaza con **menos mecanismo**: un proyecto git real auto-provisionado (`~/.octopush/sketchbook`) donde "pensar antes de construir" son threads TALK normales — cero caso NULL, cero superficie de artefactos nueva, cero monograma discontinuo. Ver `docs/superpowers/plans/2026-07-18-prompt-genesis-master-plan.md` §G5. **No implementar M4.2 por separado.**
