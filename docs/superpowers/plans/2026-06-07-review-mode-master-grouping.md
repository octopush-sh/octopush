...
- **Slice II — Navigation & ergonomics: SHIPPED 2026-06-11** (feat/review-g1-nav): tab roving tabindex + arrows/Home/End + tooltips + drag-reorder; Settings Editor section (wrap/font/tab-width/line-numbers); palette Editor group; the G3 `o` handoff closed via a store-mediated PendingReveal (openFile gained a line param — the scroll-to-line primitive G1-I never actually shipped). Note: skip setting a reveal for binary files (harmless stale entry today).
...
- **Slice II — File operations: SHIPPED 2026-06-11** (feat/review-g6-fileops): contained fs commands (rename incl. case-only on APFS, new file/folder, delete) + FileNameDialog + DANGER delete with confirm; refreshDir force-fetch covers invalidation. Follow-ups from its review: prune stale `expanded`/`children`/cache keys under a renamed dir's old path; notify `editorStore` when an open file is renamed/deleted (G2 seam — a save can resurrect a deleted file); optional toast hint when creating a gitignored file while show-ignored is off.
