...
## Amendment (2026-07-11, post-implementation)

Recorded after all four implementation plans (heart, launcher, builder, fleet + sweep) shipped, to close the gap between this spec's proposal and what actually landed.

- **Beacon handoff shipped as satisfied-by-design, not a bespoke paired fade.** §2's "signed transition" (fade-out at the old anchor synchronized with fade-in at the new one, 320ms) never needed its own CSS: `beaconAnchor()` returns one id, `octo-stage-pulse` mounts/unmounts on whichever element currently matches it, and the surrounding `<Reveal>` (decision strip) / `<FadeSwap>` (canvas state swaps) already animate the container the anchor lives in. Zero-shadow start/end of the pulse keyframe means there's never a visible "pop" when it moves. No separate handoff animation was written or is needed. Under `prefers-reduced-motion` the anchor is a static `--brass-dim` halo (see `docs/design-system.md` §6) and the "handoff" is simply which element carries that halo — instant, as specified.
- **Builder deviations from §7:** no node numbering was added (the builder never needed arabic position labels — nodes are identified by role, same as the spec implied but didn't explicitly rule out); footer validation (`✓ VALID` / first error) replaced any per-node validation chrome, exactly as written; `FlowEdge` stroke color moved to `--brass-line` for the traversed/review-loop state rather than a new dedicated builder token — no new token was needed beyond what §3 already introduced.
- **Launcher crew line shows the model only when it's overridden** from the pipeline's default — the shipped `PipelineSetup`/`StageFlow` crew-line rendering omits the model text when it matches the ticket's stored default, to keep the line quiet; it appears the moment a per-stage override is set. This wasn't called out explicitly in §6 and is recorded here as the actual behavior.
- **`§` retirement extended to the Welcome logomark**, beyond what §3's table listed (tool-call cards, Direct role headers): `WelcomeScreen`'s pre-project mark was also a rendered `§`, now replaced by the brass serif `&` (same position/sizing/treatment as the app icon). This makes `&` the single glyph doing double duty as both "Onyx & Brass" accent and app logomark — deliberate, not an accident of scope creep.
- **Fleet vocabulary (Batch A, `RunsTray` + Mission Control), not fully specified in §8:** the top-bar chip's copy is a strict if/else-if priority chain — `{n} needs you` (brass, the chip's only pulse trigger) → `{n} in flight` (quiet hairline, sage, no pulse) → `✓ {n} done` (verdigris) — and, unlike Mission Control's header, the chip carries **no combined live-cost figure**; that figure survives only in the Mission Control room header ("combined live cost across all active runs"). `CompanionCurrentRun`'s live line is a **static** current-stage-title label in a fixed S1 slot, explicitly not an activity ticker — RunFlow alone owns the live activity stream, per its own doc comment.
- **Mission Control's "Needs you" cards do not carry a `WAITING {n} MIN` eyebrow** as §8 loosely suggested; they carry a fixed-width **time-in-state** slot (`mm:ss` rolling to `Hh MMm`, tabular, S1/S2) instead, which serves the same "how long has this waited" purpose without inventing new copy chrome.
