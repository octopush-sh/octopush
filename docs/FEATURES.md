...
- **New-workspace wizard (2-step)** — "A new workspace" flow (arabic step index `1 Task & intent` · `2 Setup script`; `STEP 1 OF 2`/`STEP 2 OF 2` — Roman numerals retired app-wide). Task input → branch auto-slugged (editable override), live "BRANCH … from {base}" preview; setup-script textarea (remembered per-project); CTA "Begin". _Support:_ `src/components/WorkspaceCreator.tsx`, `workspaceStore.create` → `create_workspace` → `workspace::create`, `companionPrefsStore`. _Entry:_ per-project "+" on header.
...
- **Branch name override** — Editable branch field; auto-cleans via slugify; warns "Branch exists — the workspace will reuse it" on collision (creation then reuses that branch). _Support:_ `WorkspaceCreator.tsx`, `listBranches`.
