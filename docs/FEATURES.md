...
- **First-run crew invite (one-shot)** — The differentiator's minute-one moment: a new user who has **never started a Direct run** (all-time backend count `count_runs_all_time` — the monthly counter resets and would re-invite) sees a one-shot card floating over the **Talk canvas** (where every new user lands): *"Put a crew on it."* — five agents take on your task while you watch. **CTA** checks provider readiness first (any enabled local provider or configured key): ready → hands off to the Direct launcher with **Feature Factory preselected** via `setLauncherPrefill` (bypassing the alphabetical default) and the workspace's task as the brief — one more click and the crew is working; not ready → opens **Settings · Models** with an honest toast and the invite survives (Feature Factory is all-api and would otherwise fail mid-run). "Not now" dismisses **persistently** (`octo-first-run` localStorage); using the CTA hides it for the session. Beacon-legal (Talk has no brass live element; the CTA pulses). _Support:_ `src/components/FirstRunInvite.tsx`, `stores/firstRunStore.ts` (+ `anyProviderReady`), `App.handleSendFirstCrew`, command `count_runs_all_time` → `db.count_started_runs_all_time`. _Entry:_ automatic, Talk canvas, first-ever session.
...
- **Entitlement (premium scaffolding)** — `get_entitlement`, `direct_run_usage`, `count_runs_all_time` (first-run invite signal)
