...
### Entitlement & quota (premium scaffolding — P0)
- **Direct-runs meter** — A quiet "Direct runs · this month" count in the launcher ledger (tints toward rouge as a cap fills; P0 is uncapped so it shows just the count). _Support:_ `src/components/DirectRunsMeter.tsx`, `useEntitlement` hook, `entitlementStore`; command `direct_run_usage` → `db.count_started_runs_this_month` (runs that have *left* `draft` — i.e. been started — since the start of the month). _Entry:_ Direct launcher (`PipelineSetup`).
- **Run quota gate (inert in P0)** — `start_run` consults the current entitlement before driving a run; on a capped plan over its monthly Direct-run limit it returns `AppError::UpgradeRequired { feature, used, limit }` (frontend shows an upgrade sheet). The P0 Free entitlement grants `direct.unlimited` with no cap, so **no run is blocked today** — the gate is wired so P2 only needs to return a restricted entitlement. _Support:_ `src-tauri/src/entitlement.rs` (`Entitlement`, `check_direct_run_quota`, `FREE_DIRECT_RUNS_PER_MONTH`), gate in `commands::start_run`; command `get_entitlement`. See [`docs/premium/`](premium/).
