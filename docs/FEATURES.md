...
- **Direct-runs meter** — A quiet "Direct runs · this month" count in the launcher ledger (tints toward rouge as a cap fills; P0 is uncapped so it shows just the count). _Support:_ `src/components/DirectRunsMeter.tsx`, `useEntitlement` hook, `entitlementStore`; command `direct_run_usage` → `db.count_started_runs_this_month` (runs left `draft` since the start of the month). _Entry:_ Direct launcher (`PipelineSetup`).
