...
### AppTopBar
- **Live mascot (The Octo)** — The first element of the top bar (left of the fleet chip): a 20px `OctoMark` whose body language mirrors global app state via `useMascotState` (`src/hooks/useMascotState.ts`), priority **blocked > working > idle** — any workspace flagged in `attentionStore` → `blocked` (frozen, eyes half-mast: stillness is the signal); else any chat streaming (`chatStore.streamingByWs`) or Direct run `running`/`paused` (`runsStore`) → `working` (paddling at double tempo, eyes scanning); else `idle` (slow float, staggered paddle, blinks). Tooltip/aria reports the exact state ("Octopush — 2 agents working"). Decorative-plus-informative; no click behavior. _Support:_ `src/components/AppTopBar.tsx`; `icons/OctoMark.tsx`; `hooks/useMascotState.ts`. _Entry:_ top bar, far left.
