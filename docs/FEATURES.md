...
- **Per-stage reasoning effort (the cost/quality lever)** — Each stage carries an optional `effort` (`Effort` enum: `low·medium·high·xhigh·max`; `null`/absent = **off**, no thinking — today's default). It's **editor-set** in the pipeline builder (the Stage inspector's Reasoning control), persisted on `pipeline_stages`, copied to `run_stages` at `create_run`, and read into `StageSpec.effort` → `LlmRequest.effort`. It's the atomic knob the cost model rests on: a cheap model thinking deeply on synthesis stages, barely thinking on mechanical ones. Applied on the **API substrate only** — a CLI stage's agent manages its own reasoning, so the control is disabled and no effort pill renders for CLI stages. **Model-capability mapping** (pure, unit-tested, Anthropic): effort-capable models (Opus 4.x, Sonnet 5/4.6, Fable 5, unknown → the safe majority) get `output_config.effort:"<level>"` + `thinking:{type:"adaptive"}`, where the **level is clamped per model** (`effective_effort_level`) so we never 400 — only Opus 4.7/4.8, Sonnet 5, Fable 5, Mythos 5 accept `xhigh`; Opus 4.6 / Sonnet 4.6 (the default) fold `xhigh`→`high` but keep `max`; Opus 4.5 / unknown fold both `xhigh` and `max`→`high`. The thinking-but-not-effort models (any `haiku`, `sonnet-4-5`, `sonnet-4-0`) instead get `thinking:{type:"enabled", budget_tokens:N}` (N = low 4096 / med 8192 / high 16384 / xhigh·max 24576, clamped `[1024, max_tokens-1]`) with **no** `output_config` (they 400 on it). `temperature` is never sent (current models 400 on it with thinking). Effort also raises the `max_tokens` floor (`max_tokens_for`: none/low/med 32768 · high 48000 · xhigh/max 64000) so deep thinking doesn't truncate the answer. With thinking + tool use, the assistant turn's **full content-block array is captured verbatim and replayed in order** (`LlmResponse.raw_content` → `AssistantWithTools.raw`) so signed thinking survives and interleaved thinking/tool_use ordering is preserved (reordering 400s). OpenAI-compat maps `effort` → `reasoning_effort` (low/med/high; xhigh·max fold to high) **only on reasoning models** (o1/o3/o4, GPT-5 — omitted on gpt-4o) and clamps `max_tokens` to ≤32768 so the effort bump never exceeds a small-context ceiling. _Support:_ `providers/mod.rs` (`Effort`, `raw_content`), `providers/anthropic.rs` (`model_supports_effort`/`effective_effort_level`/`thinking_json`), `providers/openai_compat.rs` (`openai_supports_reasoning`), `orchestrator/agentic.rs` (`max_tokens_for`), `db.rs` (`effort` column on `pipeline_stages`/`run_stages`), `builder/StageInspector.tsx`. Design record: `docs/superpowers/plans/2026-07-14-per-stage-effort-design.md`.
