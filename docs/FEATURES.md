...
- **Stage nodes** — 228px card: role icon (`lib/roleIcons.ts` `iconForRole`, brass when selected else sage) + title + "{archetype} · {shortModel}" (a brass "· {effort}" is appended when reasoning effort is set); a checkpointed stage's `⟜` gate mark sits in the header, just before the icon; substrate chip (cli=purple/api=blue); 4 sage tool dots; validation marker; remove X (last-node guard). _Support:_ `StageNode.tsx`.
...
- **Launcher crew editor** — The selected ensemble's crew at rest is one quiet line (role icon + name, an overridden model in mute, a brass `· {effort}` when reasoning effort is set, `⟜` on a gated stage, `⟲ ×N` on a looping review); a pencil icon unfolds a `Reveal` editor of per-stage cards with in-place `ModelPicker` overrides (CLI locked to Anthropic), substrate chip, a brass effort chip, tool dots, gate/loop badges — launch-time model tuning only, and it folds again when the ticket selection changes. _Support:_ `direct/StageFlow.tsx`.
...
- **Drive loop** — Pick lowest non-`done` stage; complete the run if none; check budget gate then director-pause; run the stage; on a block (`ask_director`) → paused+checkpoint carrying the questions; on Failed → paused+checkpoint; on Done → auto-loop verdict decision before the gated pause. Aborts mid-stage always win.
- **DB tables** — `pipelines`, `pipeline_stages` (+ pos/parents/tools/custom_name/instructions/loop/max_iterations/effort), `runs` (+ cost/baseline/reference_model/retired/budget/linked_issue_key), `run_stages` (private per-run copy + status/tokens/cost/artifact/feedback/error/loop/diff_snapshot/session_id/resume_pending/baseline_commit/effort/blocked_questions), `roles`, `stage_log`, `stage_iterations`, `run_events`. The `effort` column (nullable TEXT, the `Effort` token; NULL = off) is additive on `pipeline_stages` and `run_stages`; `blocked_questions` (nullable TEXT, JSON `BlockedAsk`) is additive on `run_stages` (the `ask_director` escape valve).
...
- **pipeline_stages** — id, pipeline_id, position, role, agent_model, effort, substrate, checkpoint, max_iterations, pos_x, pos_y, parents, tools, custom_name, instructions, loop_target_position, loop_max_iterations, loop_mode
...
- **run_stages** — private per-run copy of stages + status, tokens, cost, artifact, feedback, error, loop_iterations, diff_snapshot, session_id, resume_pending, baseline_commit, effort
