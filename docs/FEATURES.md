...
- **Git/PR refresh cadence** — Git status refreshes on workspace/mode change, on window focus, and on a 3s interval only in run/review (skipped while hidden); the diff is built only in Review. On window focus the rail also **re-lists workspaces** (`loadAllWorkspaces`) so a workspace created out-of-band — e.g. by `octopush-mcp`'s `create_workspace` while you were in another app — appears on return. Open-PR is fetched per workspace-switch then every 60s. Token usage refreshes every 30s and on chat-turn completion.
