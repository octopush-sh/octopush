...
### Notable implementation details
- **Worktree mechanics** — `create_worktree` recycles a **stale `.git/worktrees/<branch>` slot** for the branch it's creating (prunes it with `valid+locked+working_tree`, then force-removes whatever prune leaves behind — fixing the "directory exists" failure on locked/leftover slots) and attaches the existing branch ref. Safety guards: it never deletes a live, valid worktree or the project root while clearing the target path (so it can't destroy a real checkout's uncommitted work), never touches *other* branches' slots (an unmounted/locked sibling worktree is left intact), and serializes creation process-wide so concurrent creates can't race the slot. `delete_worktree` prunes the ref but never the branch.
