...
- **DB tables** — `pipelines`, `pipeline_stages` (+ pos/parents/tools/custom_name/instructions/loop/max_iterations/effort), `runs` (+ cost/baseline/reference_model/retired/budget/linked_issue_key), `run_stages` (private per-run copy + status/tokens/cost/artifact/feedback/error/loop/diff_snapshot/session_id/resume_pending/baseline_commit/effort), `roles`, `stage_log`, `stage_iterations`, `run_events`. The `effort` column (nullable TEXT, the `Effort` token; NULL = off) is additive on `pipeline_stages` and `run_stages`.
