...
- **Stage input dossier (cross-stage handoff)** — Each stage receives the run task, a one-line pipeline breadcrumb, and the **freshest artifact of each kind** from feeding stages (transitive flow-ancestors; sibling branches don't leak), each capped at ~16K chars. **Reviewers see the actual code:** when feeding artifacts ref the worktree, the **live `git diff` is captured and embedded** in the dossier as a fenced section (capped head+tail like any section) — so `code_review`/`verify`/`test` judge the real changes, not the implementer's prose summary (best-effort: capture failure or an empty diff falls back to the "inspect with your tools" hint). _Support:_ `assemble_stage_input` (+ `git_ops::get_diff_text`), `StageInput.worktree_diff`, `user_input_for`.
