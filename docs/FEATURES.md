...
- **Pipeline authoring contract** — Same `§3.7` validator as the app's `save_pipeline` (roles, loops on review roles, acyclic parents, substrate, tools subset, maxIterations 1..100, instructions ≤8000). Per-stage inputs include the **cost/quality knobs**: `effort` (`low|medium|high|xhigh|max`; null/omit = off; API-only, auto-clamps to the model's max level at run time) and the escalation policy `escalateModel` (retry once at a stronger model on stage failure; api+cli) / `escalateEffort` (api-only). Both round-trip through the exact `StageDraft`→`save_pipeline` path the app uses; a whitespace-only `escalateModel` normalizes to none (matching `save_pipeline`), and an out-of-enum `effort`/`escalateEffort` is a clean per-stage tool error (never a panic). `describe_pipeline_schema` documents these fields, the effort/escalation **rules**, a `runtimeBehaviors` section (the `ask_director` **escape valve** — any stage may pause to ask the director when blocked, nothing to author; and **auto-escalation** guidance), and an annotated example that sets `effort` + `escalateModel`. _Support:_ `tools.rs`.
