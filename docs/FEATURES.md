...
- **Idempotent creation / reuse an existing branch** — `create_workspace` runs through the shared `workspace::create` (also used by `octopush-mcp`): it is idempotent on `(project, branch)` and **reuses an existing branch** instead of failing — the common case when continuing a teammate's or another session's work. If a workspace already exists for the branch it's returned (un-archived + worktree rebuilt if it was archived) rather than duplicated; the store upserts by id so the rail never grows a duplicate row. A branch currently checked out in another live worktree yields a friendly "checked out in another workspace" error (git can't check a branch out twice). If only the standard slot/path are occupied (not the branch itself), creation steps aside to a free slot name + path rather than failing — the persisted `worktree_path` reflects wherever it actually landed. _Support:_ `src-tauri/src/workspace.rs` (`create`, `provision_worktree`, `reuse_or_restore`), `git_ops::create_worktree` (returns the real path), `db::find_workspace_by_branch`/`set_workspace_worktree_path`.
