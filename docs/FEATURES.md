...
- **Crew notifications (native)** — A macOS notification when a Direct run **transitions** to needing the director (`→ paused`: gate/halt/budget) or **finishes** (`→ completed`, with the cost) — the first piece of "crews that work while you don't". Anti-noise contract: only on a genuine status *transition* (first sight records, never notifies), only while the window is **unfocused** (the UI is the signal otherwise), never for `aborted` (the director's own hand), and drafts becoming active are not news. Permission requested lazily on first fire; a denied permission stays quiet forever. Toggle: Settings → General → "Notify when a crew needs you" (default ON, persisted `octo-notify-prefs`). _Support:_ `src/lib/crewNotifications.ts` (`decideNotification` pure + `initCrewNotifications` listener on `run://stage-update`), `stores/notifyPrefsStore.ts`, `tauri-plugin-notification` (+ capability). _Entry:_ automatic.
